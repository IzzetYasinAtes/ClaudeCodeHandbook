...
```mermaid
flowchart TD
    START(["Kullanıcı<br/>istekte bulunur"]) --> GATHER["1. Bağlam Toplama<br/>(Context Gathering)"]
    GATHER --> PLAN["2. Planlama<br/>(Planning)"]
    PLAN --> EXEC["3. Yürütme<br/>(Execution)"]
    EXEC --> OBSERVE["4. Gözlemleme<br/>(Observation)"]
    OBSERVE --> CHECK{"Görev<br/>tamamlandı mı?"}
    CHECK -->|"Hayır"| PLAN
...
sequenceDiagram
    ...
    LLM-->>CC: Özet yanıt
    CC-->>U: "Rate limiting eklendi:<br/>- rate-limiter.ts oluşturuldu<br/>- middleware.ts güncellendi<br/>- Tüm testler geçiyor"
```
...
flowchart TD
    subgraph dosya ["Dosya Araçları"]
        F1["Read<br/>(Dosya oku)"]
        F2["Write<br/>(Dosya yaz)"]
        F3["Edit<br/>(Dosya düzenle)"]
        F4["MultiEdit<br/>(Çoklu düzenleme)"]
        F5["Glob<br/>(Dosya ara)"]
        F6["Grep<br/>(İçerik ara)"]
    end

    subgraph shell ["Shell Araçları"]
        S1["Bash<br/>(Komut çalıştır)"]
        S2["Background<br/>(Arka plan)"]
    end

    subgraph web ["Web Araçları"]
        W1["WebFetch<br/>(Web sayfası oku)"]
        W2["WebSearch<br/>(Web'de ara)"]
    end

    subgraph task_tools ["Görev Araçları"]
        T1["TodoRead<br/>(Görev listesi oku)"]
        T2["TodoWrite<br/>(Görev listesi yaz)"]
        T3["Task<br/>(Alt görev oluştur)"]
    end

    subgraph other ["Diğer Araçlar"]
        O1["LSP<br/>(Kod analizi)"]
        O2["Notebook<br/>(Jupyter)"]
        O3["MCP<br/>(Harici araçlar)"]
    end
...
```mermaid
flowchart TD
    REQ["Araç çağrısı<br/>(Tool Call)"] --> TYPE{"Araç tipi?"}

    TYPE -->|"Salt okunur"| READONLY["Otomatik izin<br/>✅ İzin gerekmez"]
    TYPE -->|"Yazma/çalıştırma"| CHECK{"İzin kuralı<br/>var mı?"}
...
flowchart LR
    subgraph baglam ["200K Token Context Window"]
        ...
        CLAUDE_MD["CLAUDE.md Kuralları<br/>(Değişken)"]
        CONV["Konuşma Geçmişi<br/>(Büyüyen)"]
        TOOL_OUT["Araç Çıktıları<br/>(Büyüyen)"]
        AVAIL["Kullanılabilir Alan<br/>(Azalan)"]
    end

    SYS --> CLAUDE_MD --> CONV --> TOOL_OUT --> AVAIL

    FULL{"Bağlam<br/>doldu mu?"} -->|"Evet"| COMPACT["Compact<br/>(Sıkıştırma)"]
    COMPACT --> SUMMARY["Önceki konuşma<br/>özetlenir"]
    SUMMARY --> CONTINUE["Yeni bağlamla<br/>devam"]
```
...
flowchart LR
    subgraph modlar ["Claude Code Modları"]
        IM["Interactive Mode<br/>(Varsayılan)<br/>Oku + Yaz + Çalıştır"]
        PM["Plan Mode<br/>Sadece Oku + Planla<br/>Yazma yok"]
        FM["Fast Mode<br/>Hızlı model<br/>Basit görevler için"]
    end
