...
flowchart LR
    subgraph swebench ["SWE-bench Süreci"]
        S1["GitHub'dan<br/>gerçek issue al"] --> S2["Kod tabanını<br/>analiz et"]
        S2 --> S3["Değişiklik<br/>planı oluştur"]
        S3 --> S4["Kodu düzenle<br/>ve test et"]
        S4 --> S5["Pull request<br/>oluştur"]
    end

    S5 --> SCORE["Otomatik<br/>değerlendirme"]
```
...
flowchart LR
    subgraph context ["Context Window Karşılaştırması"]
        C1["Claude<br/>200K<br/>(1M beta)"]
        C2["GPT-4o<br/>128K"]
        C3["Gemini 2.5 Pro<br/>2M"]
        C4["DeepSeek V3.2<br/>128K"]
    end
```
...
flowchart TD
    ...
    CTX --> CODE["Tüm Kod Tabanı<br/>Analizi"]
    CTX --> DOC["Uzun Belge<br/>İnceleme"]
    CTX --> CONV["Uzun Konuşma<br/>Geçmişi"]
    CTX --> MULTI["Çoklu Dosya<br/>Karşılaştırma"]

    CODE --> C1["Bir projenin tüm kaynak<br/>kodunu tek seferde analiz et"]
    DOC --> D1["500 sayfalık spesifikasyonu<br/>tamamen oku ve özetle"]
    CONV --> V1["Tüm proje geçmişini<br/>bağlamda tut"]
    MULTI --> M1["10 farklı dosyadaki<br/>tutarsızlıkları bul"]
```
...
flowchart TD
    ...
    VISION --> UI["UI/UX Analizi<br/>Ekran görüntüsü<br/>inceleme"]
    VISION --> CHART["Grafik Okuma<br/>Çubuk, pasta,<br/>çizgi grafikleri"]
    VISION --> DIAGRAM["Diyagram Yorumlama<br/>Akış şemaları,<br/>mimari diyagramlar"]
    VISION --> DOC["Belge Analizi<br/>Taranmış belgeler,<br/>faturalar"]
    VISION --> CODE["Kod Görüntüsü<br/>Whiteboard kodu,<br/>ekran paylaşımları"]
    VISION --> PHOTO["Fotoğraf Analizi<br/>Nesne tanıma,<br/>sahne açıklama"]
```
...
flowchart LR
    ...
    subgraph artifact ["Artifact Paneli"]
        ART["İnteraktif Çıktı<br/>- Çalışan kod<br/>- Rendered HTML<br/>- SVG grafikleri<br/>- Markdown önizleme"]
    end
...
flowchart TD
    ...
    CC --> FILE["Dosya Araçları<br/>Read, Write, Edit,<br/>MultiEdit, Glob"]
    CC --> BASH["Terminal<br/>Bash, komut çalıştırma"]
    CC --> WEB["Web<br/>Fetch, arama"]
    CC --> TASK["Görev<br/>Subagent, TodoWrite"]
    CC --> LSP["Kod Analizi<br/>LSP, diagnostics"]
    CC --> NB["Notebook<br/>Jupyter düzenleme"]
```
...
flowchart TD
    subgraph capabilities ["Claude'un Özel Yetenekleri"]
        H["Düşük Hallucination<br/>%2.8"]
        C["Kodlama Liderliği<br/>SWE-bench %80.9"]
        CTX["200K Context<br/>(1M beta)"]
        V["Vision<br/>Görsel anlama"]
        A["Artifacts<br/>İnteraktif çıktılar"]
        T["Tool Use<br/>Araç kullanımı"]
    end
```
...
flowchart TD
    ...
    NEED -->|"Güvenilir yanıtlar"| CLAUDE1["✅ Claude<br/>En düşük hallucination"]
    NEED -->|"Kod yazma/düzeltme"| CLAUDE2["✅ Claude<br/>Benchmark lideri"]
    NEED -->|"Uzun belge analizi"| MAYBE["⚖️ Claude vs Gemini<br/>Claude: 200K, Gemini: 2M"]
    NEED -->|"Ses/video işleme"| OTHER["⚠️ Gemini veya GPT<br/>Claude multimodal girdide sınırlı"]
    NEED -->|"Güvenlik kritik"| CLAUDE3["✅ Claude<br/>Constitutional AI"]
    NEED -->|"Terminal AI Agent"| CLAUDE4["✅ Claude Code<br/>En gelişmiş AI agent"]
```
