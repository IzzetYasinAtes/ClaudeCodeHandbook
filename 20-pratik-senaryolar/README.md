...
```mermaid
flowchart TD
    A["Proje Keşfetme\nYeni kod tabanını anlama"] --> B["Bug Düzeltme\nHata bulma ve çözme"]
    B --> C["Refactoring\nKodu yeniden yapılandırma"]
    C --> D["Test Yazma\nOtomatik test oluşturma"]
    D --> E["Dokümantasyon\nOtomatik dokümantasyon"]
    E --> F["Sıfırdan Proje\nKomple proje oluşturma"]
    F --> G["API Geliştirme\nREST/GraphQL API"]
    G --> H["Veritabanı İşlemleri\nMigration ve schema"]
    H --> I["Performans Optimizasyonu\nDarboğaz tespiti"]
```
...
```mermaid
flowchart LR
    A{"Ne yapmak\nistiyorsunuz?"} -->|"Yeni projeye\nkatıldım"| B["01-Proje Keşfetme"]
    A -->|"Hata var,\ndüzeltmem gerekiyor"| C["02-Bug Düzeltme"]
    A -->|"Kod karmaşık,\nsadeleştirmem gerekiyor"| D["03-Refactoring"]
    A -->|"Test yazmam\ngerekiyor"| E["04-Test Yazma"]
    A -->|"Dokümantasyon\nexik"| F["05-Dokümantasyon"]
    A -->|"Sıfırdan proje\nbaşlıyorum"| G["06-Sıfırdan Proje"]
    A -->|"API geliştirmem\ngerekiyor"| H["07-API Geliştirme"]
    A -->|"Veritabanı\nişlemleri"| I["08-Veritabanı"]
    A -->|"Uygulama yavaş\nçalışıyor"| J["09-Performans"]
```
