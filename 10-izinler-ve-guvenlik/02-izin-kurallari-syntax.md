...
flowchart LR
    subgraph genel ["Genel Eşleştirme"]
        A["Bash"] --> A1["TÜM bash komutlarını<br/>eşleştirir"]
        B["WebFetch"] --> B1["TÜM web isteklerini<br/>eşleştirir"]
        C["Read"] --> C1["TÜM dosya okumalarını<br/>eşleştirir"]
    end

    subgraph ozel ["Belirli Eşleştirme"]
        D["Bash(npm test)"] --> D1["Sadece 'npm test'<br/>komutunu eşleştirir"]
        E["Read(./.env)"] --> E1["Sadece .env dosyasını<br/>eşleştirir"]
        F["WebFetch(domain:api.com)"] --> F1["Sadece api.com<br/>alanını eşleştirir"]
    end
```
...
```mermaid
flowchart TD
    START["Araç Çağrısı<br/>örn: Bash(npm run build)"] --> DENY{"1. deny listesi<br/>kontrol et"}

    DENY -->|"Eşleşme var"| DENIED["❌ Reddedildi<br/>Sorulmadan engellenir"]
    DENY -->|"Eşleşme yok"| ASK{"2. ask listesi<br/>kontrol et"}

    ASK -->|"Eşleşme var"| PROMPT["🔒 Kullanıcıya Sor<br/>Onay bekle"]
    ASK -->|"Eşleşme yok"| ALLOW{"3. allow listesi<br/>kontrol et"}
...
```mermaid
flowchart TD
    A["Enterprise Policy<br/>(organizasyon politikası)"] --> B["~/.claude/settings.json<br/>(global kullanıcı ayarları)"]
    B --> C[".claude/settings.json<br/>(proje ayarları)"]
    C --> D["Oturum içi izinler<br/>(geçici)"]
