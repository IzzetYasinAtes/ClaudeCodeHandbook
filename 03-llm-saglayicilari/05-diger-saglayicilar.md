...
flowchart TD
    ...
    D --> DS["DeepSeek 🇨🇳<br/>Açık kaynak reasoning"]
    D --> QW["Qwen (Alibaba) 🇨🇳<br/>Çok dilli, açık kaynak"]
    D --> MI["Mistral 🇫🇷<br/>Avrupa uyumlu"]
    D --> PH["Microsoft Phi 🇺🇸<br/>Küçük & güçlü"]
...
flowchart TD
    ...
    DS --> R["DeepSeek-R1<br/>(Reasoning)"]
    DS --> V["DeepSeek-V3.2<br/>(Genel Amaçlı)"]
    DS --> CODER["DeepSeek-Coder<br/>(Kodlama)"]

    R --> R1["R1<br/>671B (MoE)<br/>37B aktif"]
    R --> R1D["R1-Distill<br/>1.5B - 70B<br/>(Damıtılmış)"]

    V --> V32["V3.2<br/>685B (MoE)<br/>37B aktif"]
...
flowchart LR
    subgraph geleneksel ["Geleneksel Dense Model"]
        T1["GPT-5<br/>Tüm parametreler aktif<br/>Yüksek hesaplama maliyeti"]
    end

    subgraph moe ["DeepSeek MoE"]
        T2["671B toplam parametre<br/>Ama sadece 37B aktif<br/>Düşük hesaplama maliyeti"]
    end

    T1 -.->|"~10x daha<br/>pahalı çıkarım"| T2
```
...
flowchart TD
    ...
    QW --> Q35["Qwen 3.5<br/>(En Güncel)"]
    QW --> Q25["Qwen 2.5<br/>(Stabil)"]
    QW --> QC["Qwen2.5-Coder<br/>(Kod Uzmanı)"]
    QW --> QV["Qwen-VL<br/>(Vision-Language)"]

    Q35 --> Q35_72["72B"]
    Q35 --> Q35_122["122B ⭐<br/>(En büyük)"]
...
flowchart TD
    ...
    ACIK --> M7["Mistral 7B<br/>(İlk model)"]
    ACIK --> MIX["Mixtral 8x22B<br/>(MoE öncüsü)"]
    ACIK --> MS["Mistral Small<br/>(24B)"]

    KAPALI --> ML["Mistral Large 2<br/>(123B) ⭐"]
...
```mermaid
flowchart LR
    P1["Phi-1<br/>(1.3B)<br/>2023"]
    P2["Phi-2<br/>(2.7B)<br/>2023"]
    P3["Phi-3<br/>(3.8B-14B)<br/>2024"]
    P4["Phi-4<br/>(14B) ⭐<br/>2025"]
...
flowchart TD
    ...
    PHI --> EDGE["Edge Computing<br/>(Uç cihazlar)"]
    PHI --> MOB["Mobil Uygulamalar"]
    PHI --> EMB["Embedded Systems<br/>(Gömülü sistemler)"]
    PHI --> ED["Eğitim & Araştırma<br/>(Düşük donanım)"]

    EDGE --> EDGE1["Lokal çalıştırma<br/>İnternet gerektirmez"]
    MOB --> MOB1["Telefonda AI<br/>Düşük gecikme"]
```
...
flowchart LR
    subgraph acik ["Açık Kaynak"]
        DS2["DeepSeek<br/>MIT"]
        QW2["Qwen<br/>Apache 2.0"]
        PH2["Phi<br/>MIT"]
        YI2["Yi<br/>Apache 2.0"]
    end

    subgraph karma ["Karma"]
        MI2["Mistral<br/>Açık + Kapalı"]
    end

    subgraph kapali ["Kapalı / Kurumsal"]
        CO2["Cohere<br/>API only"]
    end
```
