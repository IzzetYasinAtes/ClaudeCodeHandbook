...
flowchart TB
    subgraph traditional ["Geleneksel İnceleme"]
        ...
        T2["Sınırlı kapsam\n(sadece diff)"]
        T3["Subjektif kalite\n(kişiye bağlı)"]
        T4["Tutarsız standart\n(incelemeciye göre değişir)"]
    end

    subgraph ai ["AI Destekli İnceleme"]
        direction TB
        A1["Anında inceleme\n(dakikalar)"]
        A2["Tam codebase bağlamı\n(diff + tüm proje)"]
        A3["Objektif analiz\n(tutarlı kriterler)"]
        A4["Çok boyutlu\n(güvenlik, performans, mantık)"]
    end
...
flowchart TB
    ...
    subgraph layers ["İnceleme Katmanları"]
        direction TB
        L1["1️⃣ Söz Dizimi & Stil\nFormatting, naming, conventions"]
        L2["2️⃣ Mantık Hataları\nLogic errors, edge cases"]
        L3["3️⃣ Güvenlik Analizi\nInjection, auth bypass, secrets"]
        L4["4️⃣ Performans\nN+1, memory leak, complexity"]
        L5["5️⃣ Mimari Uyum\nDesign patterns, SOLID"]
        L6["6️⃣ Test Yeterliliği\nCoverage, edge cases, mocking"]
    end
...
```mermaid
flowchart TB
    PR["Pull Request\n(50+ dosya değişikliği)"]

    PR --> ORCH["Orchestrator Agent\n(Koordinatör)"]

    ORCH --> SA1["Security Agent\n🔒 Güvenlik analizi"]
    ORCH --> SA2["Logic Agent\n🧠 Mantık kontrolü"]
    ORCH --> SA3["Performance Agent\n⚡ Performans analizi"]
    ORCH --> SA4["Test Agent\n🧪 Test yeterliliği"]
...
```mermaid
flowchart LR
    PR["PR: API Endpoint"] --> SEC["Güvenlik\nAnalizi"]
    SEC --> R1["🔒 Auth check?"]
