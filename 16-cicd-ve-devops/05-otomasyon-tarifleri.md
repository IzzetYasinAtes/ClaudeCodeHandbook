...
flowchart TB
    ...
    BRANCH --> IMPLEMENT["🛠️ Değişiklikleri\nUygula"]
    IMPLEMENT --> TEST["🧪 Testleri\nÇalıştır"]
    
    TEST --> CHECK{"Testler\ngeçiyor mu?"}
    CHECK -->|Evet| CREATE_PR["✅ PR Oluştur"]
    CHECK -->|Hayır| FIX["🔧 Hataları\nDüzelt"]
    FIX --> TEST
    
    CREATE_PR --> COMMENT["💬 Issue'ya\nYorum Yaz"]
...
```mermaid
flowchart LR
    PR["PR Merged\nmain branch"] --> TRIG["Workflow\nTetiklenir"]
    TRIG --> ANALYZE["Değişiklikleri\nAnaliz Et"]
    ANALYZE --> UPDATE["Dokümantasyonu\nGüncelle"]
...
```mermaid
flowchart TB
    TRIG["Manuel Tetikleme\nveya Schedule"]
    TRIG --> COV["📊 Mevcut Coverage\nRaporu Al"]
    COV --> ANALYZE["🔍 Coverage Gap\nAnalizi"]
    ANALYZE --> GEN["🧪 Test\nOluştur"]
    GEN --> RUN["▶️ Testleri\nÇalıştır"]
    RUN --> CHECK{"Hedef\nKarşılandı mı?"}
...
```mermaid
flowchart LR
    TAG["🏷️ Yeni Tag\nv1.2.0"] --> TRIG["Workflow"]
    TRIG --> COMMITS["📜 Commit\nGeçmişi"]
    COMMITS --> ANALYZE["🔍 Değişiklikleri\nKategorize Et"]
    ANALYZE --> NOTES["📝 Release Notes\nOluştur"]
