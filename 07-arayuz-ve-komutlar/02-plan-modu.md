...
```mermaid
flowchart TD
    START["Görev alındı"] --> ANALYZE{Görev<br/>karmaşık mı?}
    
    ANALYZE -->|"Basit"| DIRECT["Doğrudan Normal Mode<br/>ile çöz"]
    ANALYZE -->|"Karmaşık"| PLAN["Plan Mode'a geç<br/>(Shift+Tab)"]
    ...
    PL4 --> APPROVE{Plan<br/>onaylandı mı?}
    APPROVE -->|"Hayır"| REVISE["Planı revize et"]
    REVISE --> PL1
    APPROVE -->|"Evet"| IMPLEMENT["Normal Mode'a geç<br/>(Shift+Tab)"]
    
    subgraph impl ["🔨 Uygulama (Normal Mode)"]
        IMPLEMENT --> I1["Plana göre<br/>kodu değiştir"]
...
flowchart LR
    ...
    plansiz -.->|"Daha fazla token<br/>Daha fazla hata"| COST1["💰💰💰"]
    planli -.->|"Daha az token<br/>Daha az hata"| COST2["💰"]
```
...
flowchart TD
    ...
    F1 --> D1["Yanlış dosyayı düzenleme<br/>riskini ortadan kaldırır"]
    F2 --> D2["Gereksiz yazma/geri alma<br/>döngüsünü önler"]
    F3 --> D3["Tüm bağımlılıkları ve<br/>etkilenen dosyaları bulur"]
    F4 --> D4["Adım adım uygulama<br/>planı sunar"]
    F5 --> D5["Olası sorunları önceden<br/>tespit eder"]
```
