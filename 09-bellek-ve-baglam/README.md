...
```mermaid
flowchart TD
    A["CLAUDE.md Dosyası<br/>(Birincil Bellek)"] --> B["AGENTS.md ve<br/>Diğer Config"]
    B --> C["Kurallar Dizini<br/>(.claude/rules/)"]
    C --> D["Otomatik Bellek<br/>(Auto Memory)"]
    D --> E["Context Window<br/>Yönetimi"]
    E --> F["Oturum Yönetimi"]
    F --> G["Checkpointing"]
...
flowchart LR
    subgraph kotu ["❌ Kötü Bağlam Yönetimi"]
        ...
        K2 --> K3["Her seferinde<br/>aynı şeyleri tekrarlıyor"]
        K3 --> K4["Verim düşük"]
    end
